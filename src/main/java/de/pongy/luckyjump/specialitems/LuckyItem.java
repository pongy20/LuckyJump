...
public class LuckyItem {
    ...
    public void giveItem(Player player) {
        if (lucky) {
            player.sendMessage(ChatColor.GREEN + "You've got lucky!");
            player.sendMessage(ChatColor.GREEN.toString() + ChatColor.BOLD + "You've got " + ChatColor.GOLD.toString() + ChatColor.UNDERLINE + name + ChatColor.RESET + ChatColor.GREEN.toString() + ChatColor.BOLD);
            player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 10, 10);
        } else {
            player.sendMessage(ChatColor.RED + "You got unlucky");
            player.sendMessage(ChatColor.RED.toString() + ChatColor.BOLD + "You've got " + ChatColor.GOLD.toString() + ChatColor.UNDERLINE + name + ChatColor.RESET + ChatColor.RED.toString() + ChatColor.BOLD);
